...
BOOL MxHidDevice::DCDWrite(PUCHAR DataBuf, UINT RegCount)
{
    ...
    SDPCmd.data = 0;
    SDPCmd.address = 0;
	if ((GetDevType() != MX6Q) && ( GetDevType() != MX6S )) 
	{
		while(RegCount)
		{
	...
	else
	{
		...
		while(RegCount)
		{
			...
			if (!SendCmd(&SDPCmd)) {
				_tprintf(_T("Major Error: write to device failed.\n") );
				return FALSE;
			}
			
			if (!SendData(DataBuf, SDPCmd.dataCount))
			{
				_tprintf(_T("Major Error: send data failed.\n"));
				return FALSE;
			}	

			_tprintf(_T("SendData Success.\n"));
		...
		for(int i=0; i<RegCount; i++)
		{
			...
			// _tprintf(_T("Reg 0x%x is initialized.\n"), EndianSwap(pMemPara[i].addr));
		}
	}
	_tprintf(_T("DCD Regs Written....\n"));
