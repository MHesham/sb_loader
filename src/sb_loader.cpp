...
int MxRun(CString fwFilename,UCHAR* DataBuf,ULONGLONG fwSize, MxHidDevice::PMxFunc pMxFunc)
{
	...
	if (g_pMxHidDevice == NULL)
	{
		// major error, we'll crash when we try to run the plugin.
		_tprintf(_T("%s Major Error, no g_pMxHidDevice.\n"),indent, fwFilename);
		exit(1);
	}

	//
	// Execute plugin (or load DCD table)
	//
